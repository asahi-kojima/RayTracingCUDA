...
public:
	...
	__device__ __host__ Ray transform(const Mat4& mat) const
	{
		...
		return Ray(transformedOrigin.extractXYZ(), transformedDirectin.extractXYZ().normalize());
	}
	__device__ __host__ Ray transformWithoutNormalize(const Mat4& mat) const
	{
		const Vec4 origin(mOrigin, 1);
		const Vec4 direction(mDirection, 0);

		const Vec4 transformedOrigin = mat * origin;
		const Vec4 transformedDirection = mat * direction;

		return Ray(transformedOrigin.extractXYZ(), transformedDirection.extractXYZ());
	}
